...
warnings.filterwarnings(
    "ignore", category=DeprecationWarning, module="langchain_pinecone"
)
        ...
        if self._pinecone_index is None:
            ...
            try:
                ...
                if "ALREADY_EXISTS" in str(e):
                    logger.info(
                        f"Index {PINECONE_INDEX_NAME} already exists, proceeding with existing index"
                    )
                    self._pinecone_index = pc.Index(PINECONE_INDEX_NAME)
                else:
        ...
        try:
            ...
            if (
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.warning(
                        f"LLM rate limit hit. Waiting {wait_time} seconds before retry {attempt + 1}/{max_retries}"
                    )
...
def create_pinecone_vectors(chunks, note_id, user_id, batch_size=5):
    ...
    try:
        ...
        for i in range(0, len(chunks), batch_size):
            ...
            if not embeddings:
                raise ValueError(
                    f"No embeddings generated for batch starting at index {i}"
                )
            ...
            logger.info(
                f"Upserted batch {(i // batch_size) + 1} of {(len(chunks) + batch_size - 1) // batch_size}"
            )
        ...
        if chunks:
            ...
            results = pinecone_index.query(
                vector=test_emb, top_k=1, namespace=namespace, include_metadata=True
            )
            if results.matches and abs(results.matches[0].score - 1.0) < 0.1:
                ...
                logger.warning(
                    f"Verification failed: {len(results.matches)} matches found"
                )
                return False, None
...
"""
        try:
            ...
            return markdown_to_html(
                "Error generating summary: Limit reached or content error."
            )
...
def upload_to_cloudinary(file_content, resource_type, folder):
    ...
    try:
        ...
        upload_result = cloudinary.uploader.upload(
            file_stream, resource_type=resource_type, folder=folder
        )
    ...
    try:
        ...
        try:
            ...
            return (
                jsonify(
                    {
                        "error": f"Failed to process signup due to database error: {str(e)}"
                    }
                ),
                500,
            )
...
def verify_otp():
    try:
        ...
        try:
            ...
            return (
                jsonify(
                    {"error": f"Failed to verify OTP due to database error: {str(e)}"}
                ),
                400,
            )
        except httpx.ConnectError as e:
            logger.error(
                f"Connection error to Supabase during OTP verification: {str(e)}"
            )
            return jsonify({"error": f"Failed to connect to database: {str(e)}"}), 503
    ...
    try:
        ...
        try:
            ...
            return (
                jsonify(
                    {
                        "error": f"Failed to process login due to database error: {str(e)}"
                    }
                ),
                500,
            )
...
def upload_pdf():
    ...
    try:
        ...
        logger.info(
            f"Starting PDF upload for user {user_id}. File: {request.files.get('file').filename if 'file' in request.files else 'None'}"
        )
        ...
        try:
            ...
            logger.info(
                f"User {user_id} has uploaded {pdf_count} PDFs in the last 24 hours"
            )
            if pdf_count >= 5:
            ...
            return (
                jsonify({"error": f"Database error checking upload limit: {str(e)}"}),
                500,
            )
        except httpx.ConnectError as e:
            logger.error(f"Connection error to Supabase: {str(e)}")
            ...
            return (
                jsonify({"error": f"Unexpected error checking upload limit: {str(e)}"}),
                500,
            )
        ...
        try:
            ...
            return (
                jsonify({"error": f"Failed to upload PDF to Cloudinary: {str(e)}"}),
                500,
            )
        ...
        try:
            ...
            return (
                jsonify({"error": f"Failed to store PDF note in database: {str(e)}"}),
                500,
            )
        except httpx.ConnectError as e:
        ...
        logger.info(
            f"Creating vector store with {len(chunks)} chunks using HF embeddings..."
        )
        try:
        ...
        if success and namespace:
            try:
                supabase.table("notes").update({"pinecone_namespace": namespace}).eq(
                    "id", note_id
                ).execute()
                logger.info(
                    f"Successfully created Pinecone vector store for note {note_id}"
                )
            except postgrest.exceptions.APIError as e:
                logger.error(f"Supabase API error updating namespace: {str(e)}")
                return (
                    jsonify(
                        {"error": f"Failed to update vector store namespace: {str(e)}"}
                    ),
                    500,
                )
            except httpx.ConnectError as e:
                logger.error(
                    f"Connection error to Supabase updating namespace: {str(e)}"
                )
                return (
                    jsonify({"error": f"Failed to connect to database: {str(e)}"}),
                    503,
                )
        else:
...
def delete_note(note_id):
    try:
        ...
        try:
            supabase.table("chats").delete().eq("note_id", note_id).eq(
                "user_id", user_id
            ).execute()
            supabase.table("notes").update(
                {"deleted": True, "file_url": None, "pinecone_namespace": None}
            ).eq("id", note_id).eq("user_id", user_id).execute()
            logger.info(
                f"Note {note_id} and associated chats soft-deleted for user {user_id}"
            )
        except postgrest.exceptions.APIError as e:
            ...
            return (
                jsonify(
                    {
                        "error": f"Failed to soft-delete note due to database error: {str(e)}"
                    }
                ),
                500,
            )
        ...
        return (
            jsonify(
                {"message": "Note and all associated data deleted successfully from UI"}
            ),
            200,
        )
...
def chat():
    try:
        ...
        try:
            ...
            return (
                jsonify({"error": f"Unexpected error checking chat limit: {str(e)}"}),
                500,
            )
        ...
        if note_id:
            try:
                ...
                if note_result.data:
                    ...
                    logger.info(
                        f"Attempting to retrieve relevant chunks for note {note_id}..."
                    )
                    try:
                        relevant_chunks = get_relevant_chunks(
                            note_id, user_id, message, top_k=5
                        )
                        if relevant_chunks:
                            ...
                            logger.info(
                                "No relevant chunks retrieved, using fallback context"
                            )
                ...
                try:
                    ...
                    logger.error(
                        f"Supabase API error retrieving chat history: {str(e)}"
                    )
                    return (
                        jsonify(
                    ...
                    logger.error(
                        f"Connection error to Supabase retrieving chat history: {str(e)}"
                    )
                    return (
                        jsonify({"error": f"Failed to connect to database: {str(e)}"}),
                        503,
...
"""
            try:
                ...
                logger.error(f"Gemini blocked prompt error: {str(e)}")
                return markdown_to_html(
                    "Sorry, the query was blocked due to content restrictions."
                )
            except Exception as e:
                logger.error(f"Gemini response generation failed: {str(e)}")
                return markdown_to_html(
                    "Error generating response: Limit reached or content error."
                )
...
def rename_note(note_id):
    try:
        ...
        try:
            supabase.table("notes").update({"title": new_title}).eq("id", note_id).eq(
                "user_id", user_id
            ).execute()
...
def get_chat_history(note_id):
    try:
        ...
        try:
            ...
            logger.info(
                f"Retrieved {len(history)} chat messages for user {user_id}, note {note_id}"
            )
...
def change_password():
    try:
        ...
        if not email or not old_password or not new_password:
            logger.warning(
                "Missing email, old password, or new password in change-password request"
            )
            return (
                jsonify(
        ...
        try:
            ...
            logger.error(
                f"Connection error to Supabase verifying credentials: {str(e)}"
            )
            return jsonify({"error": f"Failed to connect to database: {str(e)}"}), 503
...
def export_note_to_pdf(note_id):
    try:
        ...
        def parse_html_tag(tag, style, code_style, highlight_style):
            ...
            if tag.name == "p":
                ...
                return ListFlowable(
                    ...
                    start="disc" if tag.name == "ul" else 1,
                    leftIndent=20,
                )
            elif tag.name == "li":
                marked_up = html_to_reportlab(tag)
                ...
                for child in tag.children:
                    child_elem = parse_html_tag(
                        child, style, code_style, highlight_style
                    )
                    if child_elem:
        ...
        def parse_html_content(
            html_content, content_style, code_style, highlight_style
        ):
            if not html_content:
        ...
        story.extend(
            parse_html_content(note["summary"], body_style, code_style, highlight_style)
        )
        ...
        if chats:
            for i, chat in enumerate(chats, 1):
                ...
                story.extend(
                    parse_html_content(
                        chat["ai_response"], answer_style, code_style, highlight_style
                    )
                )
                story.append(Spacer(1, 36))
        else:
